...
std::string init_rc() {
  ...
  if(fd_env == -1){
    ...
    //buf_env;
    buf_env[count_env-2]='\0';
    memmove (buf_env, buf_env+6, strlen (buf_env+6) + 6);
    //std::cout<<buf_env;
    close(fd_env);
  }
  /* fetching hostname */
  ...
  write_to_rc_string+=convertToString(_path,strlen(_path));
...
if(is_file_exsists("shellrc")){
  //yes read file
  
}else{
  //write
    std::fstream f_stream_obj;

    f_stream_obj.open("shellrc", std::ios::trunc | std::ios::out | std::ios::in); 
  
    f_stream_obj << init_rc(); 
